...
function getDisplayPair(catalog, skill) {
  return {
    description: catalog[`codex-skills.${skill}.description`],
    shortDescription: catalog[`codex-skills.${skill}.short-description`],
  };
}
...
describe('codex skill display localization catalog', () => {
  test('zh-CN catalog mirrors the English canonical key set exactly', () => {
  ...
  test('zh-CN catalog keeps the official six-skill boundary only', () => {
    const chineseCatalog = readCatalog(ZH_CATALOG_PATH);
