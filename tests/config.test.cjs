...
describe('config-path command (#2282)', () => {
  ...
  test('returns root config path when no workstream is active', () => {
    ...
    const configPath = toRepoPath(result.output.trim());
    assert.ok(configPath.endsWith('.planning/config.json'), `expected root config path, got: ${result.output}`);
    assert.ok(!configPath.includes('workstreams'), 'should not include workstreams in path');
  });
  ...
  test('returns workstream config path when GSD_WORKSTREAM is set', () => {
    ...
    assert.ok(result.success, `config-path failed: ${result.error}`);
    const configPath = toRepoPath(result.output.trim());
    assert.ok(configPath.includes('workstreams/my-stream/config.json'), `expected workstream config path, got: ${result.output}`);
  });
