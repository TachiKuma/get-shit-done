...
**If `response_language` is set:** Treat it as a normalized canonical locale (for example `en`, `zh-CN`, `ja-JP`, `ko-KR`, `pt-BR`) and include `response_language: {value}` in all spawned subagent prompts so any user-facing output stays in the configured language.

Fixed-string template labels and other reusable planning assets belong to the `assets` locale namespace, while plan-specific narrative remains generated via `response_language`. Commands, paths, code, identifiers, and key technical terms remain in English.
