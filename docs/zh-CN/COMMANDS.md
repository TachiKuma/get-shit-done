...
### `/gsd-edit-phase <N> [--force]`

- 原地编辑 ROADMAP.md 中现有阶段的任意字段，阶段编号和位置保持不变。
- 支持编辑单个字段（标题、描述/目标、需求、成功标准、depends_on），或从澄清后的意图重新生成所有字段。
- 默认拒绝编辑进行中/已完成的阶段，除非传入 `--force` 标志。
- 写入前显示差异并请求确认。
- 典型用途：在规划阶段前修正阶段目标措辞、更新依赖关系或调整成功标准。
