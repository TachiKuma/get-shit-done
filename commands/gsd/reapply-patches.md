...
if [ -n "$KILO_CONFIG_DIR" ]; then
  candidate="$(expand_home "$KILO_CONFIG_DIR")/gsdcn-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -n "$KILO_CONFIG" ]; then
  candidate="$(dirname "$(expand_home "$KILO_CONFIG")")/gsdcn-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -n "$XDG_CONFIG_HOME" ]; then
  candidate="$(expand_home "$XDG_CONFIG_HOME")/kilo/gsdcn-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$OPENCODE_CONFIG_DIR" ]; then
  candidate="$(expand_home "$OPENCODE_CONFIG_DIR")/gsdcn-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -z "$PATCHES_DIR" ] && [ -n "$OPENCODE_CONFIG" ]; then
  candidate="$(dirname "$(expand_home "$OPENCODE_CONFIG")")/gsdcn-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -z "$PATCHES_DIR" ] && [ -n "$XDG_CONFIG_HOME" ]; then
  candidate="$(expand_home "$XDG_CONFIG_HOME")/opencode/gsdcn-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$GEMINI_CONFIG_DIR" ]; then
  candidate="$(expand_home "$GEMINI_CONFIG_DIR")/gsdcn-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$CODEX_HOME" ]; then
  candidate="$(expand_home "$CODEX_HOME")/gsdcn-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$CLAUDE_CONFIG_DIR" ]; then
  candidate="$(expand_home "$CLAUDE_CONFIG_DIR")/gsdcn-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/gsdcn-local-patches" ]; then
    PATCHES_DIR="$HOME/.config/kilo/gsdcn-local-patches"
  elif [ -d "$HOME/.config/opencode/gsdcn-local-patches" ]; then
    PATCHES_DIR="$HOME/.config/opencode/gsdcn-local-patches"
  elif [ -d "$HOME/.opencode/gsdcn-local-patches" ]; then
    PATCHES_DIR="$HOME/.opencode/gsdcn-local-patches"
  elif [ -d "$HOME/.gemini/gsdcn-local-patches" ]; then
    PATCHES_DIR="$HOME/.gemini/gsdcn-local-patches"
  elif [ -d "$HOME/.codex/gsdcn-local-patches" ]; then
    PATCHES_DIR="$HOME/.codex/gsdcn-local-patches"
  else
    PATCHES_DIR="$HOME/.claude/gsdcn-local-patches"
  fi
fi
# Local install fallback — check all runtime directories
if [ ! -d "$PATCHES_DIR" ]; then
  for dir in .config/kilo .kilo .config/opencode .opencode .gemini .codex .claude; do
    if [ -d "./$dir/gsdcn-local-patches" ]; then
      PATCHES_DIR="./$dir/gsdcn-local-patches"
      break
    fi
  done
fi
...
Local patches are automatically saved when you run /gsdcn-update
...
```bash
PRISTINE_DIR="$CONFIG_DIR/gsdcn-pristine"
```
...
Rerun /gsdcn-reapply-patches to retry with full verification.
```
