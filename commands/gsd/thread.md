...
```
No threads found. Create one with: /gsdcn-thread <description>
```
...
2. Read the file and display a summary:
   ...
   ─────────────────────────────────────
   Resume with: /gsdcn-thread {SLUG}
   Close with:  /gsdcn-thread close {SLUG}
   ```
...
6. Report:
   ...
   Resume anytime with: /gsdcn-thread {slug}
   Close when done with: /gsdcn-thread close {slug}
   ```
</mode_create>
...
- Lighter weight than /gsdcn-pause-work — no phase state, no plan context
- The value is in Context and Next Steps — a cold-start session can pick up immediately
- Threads can be promoted to phases or backlog items when they mature:
  /gsdcn-add-phase or /gsdcn-add-backlog with context from the thread
