...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-sdk query`. Use `node $HOME/.claude/get-shit-done/bin/gsdcn-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-sdk query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  node $HOME/.claude/get-shit-done/bin/gsdcn-tools.cjs config-set graphify.enabled true

Then run /gsdcn-graphify build to create the initial graph.
```
...
```bash
node $HOME/.claude/get-shit-done/bin/gsdcn-tools.cjs graphify query <term>
```

Parse the JSON output and display results:
- If the output contains `"disabled": true`, display the disabled message from Step 1 and **STOP**
- If the output contains `"error"` field, display the error message and **STOP**
- If no nodes found, display: `No graph matches for '<term>'. Try /gsdcn-graphify build to create or rebuild the graph.`
...
```bash
node $HOME/.claude/get-shit-done/bin/gsdcn-tools.cjs graphify status
```
...
```bash
node $HOME/.claude/get-shit-done/bin/gsdcn-tools.cjs graphify diff
```
...
```
PREFLIGHT=$(node "$HOME/.claude/get-shit-done/bin/gsdcn-tools.cjs" graphify build)
```
...
Project root: ${CWD}
gsd-tools path: $HOME/.claude/get-shit-done/bin/gsdcn-tools.cjs
...
4. **Write diff snapshot:**
   ```
   node \"$HOME/.claude/get-shit-done/bin/gsdcn-tools.cjs\" graphify build snapshot
   ```
