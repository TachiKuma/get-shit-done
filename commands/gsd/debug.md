...
─────────────────────────────────────────────
Run `/gsdcn-debug continue <slug>` to resume a session.
No sessions? `/gsdcn-debug <description>` to start.
```

If no files exist or the glob returns nothing: print "No active debug sessions. Run `/gsdcn-debug <issue description>` to start one."
...
Check `.planning/debug/{SLUG}.md` exists. If not, print "No active debug session found with slug: {SLUG}. Check `/gsdcn-debug list` for active sessions." and stop.
...
If summary shows `ABANDONED`: note session saved at `.planning/debug/{slug}.md` for later `/gsdcn-debug continue {slug}`.
